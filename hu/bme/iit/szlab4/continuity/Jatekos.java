...
public class Jatekos {
	...
	private final int szam; // Ebből a paraméterből tudja majd a játékos, hogy melyik billentyűkre kell reagálnia.
	...
	//Ez a függvény kezeli a tile-ok között történő mozgásért.
	public void atmegy(Tile t){
		//ha a céltile tőlünk jobbra van ...
		if (t.getKoord().x>aktTile.getKoord().x)
		{
			//az új tile bal oldalára kerülünk
			aktKoord.x = meret/2;			
		}
		//ha a céltile tőlünk balra van ...
	...
	public void meghal(){
		//Az új aktuális tile a kezdőtile
		aktTile=kezdoTile;
		//Az új aktuális koord a kezdőkoord
		aktKoord=new Pozicio(kezdoKoord.x,kezdoKoord.y);
		ugras = 0;
		xSeb = 0;
		ugrik = false;
	}
	//Ez a függvény mozgatja majd a játékost a lenyomott billentyű függvényében
	public void reszletesMozgat(Billentyu b){
		//jobbra mozgás
		//a feltétel: HA az 1-es játékosról van szó, akkor a j1_jobb gombot nézzük, ha a 2-esről, akkor a j2_jobbot
		...
		//balra mozgás
		//a feltétel: HA az 1-es játékosról van szó, akkor a j1_bal gombot nézzük, ha a 2-esről, akkor a j2_balt
		else if(((szam==1)&&(b.j1_bal==true) || (szam==2)&&(b.j2_bal)))
		{
			...
			//MÓDOSÍTJUK A SZÖGET
			if(ugras >= 0){	//csak az Y komponens érvényes a sebességből
	...
	public void ugras(Billentyu b){
		...
		if(szam == 1){	//1. Játékos
				...
				if(ugras>=-20)
					ugras -= grav;
				//aktuális függőleges irányú koordinátához hozzáadjuk az ugrás értékét
				aktKoord.y += ugras;
				ugrik = true;
				...
				// majd az előző x sebességgel és a módosított y sebességgel újraszámoljuk az irányt
				if(xSeb==0){
					if(ugras >= 0){	//csak az Y komponens érvényes a sebességből
						szog = 90;	//ha az felfelé mutat...
					} else {
